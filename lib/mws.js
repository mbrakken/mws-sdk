...
function AmazonMwsClient(accessKeyId, secretAccessKey, merchantId, options) {
	...
	this.creds = tls.createSecureContext(options.creds || {}) || crypto.createCredentials(options.creds || {});
...
AmazonMwsRequest.prototype.set = function(param, value) {
	console.log('AmazonMwsRequest.prototype.set:', param, value);
...
AmazonMwsRequest.prototype.query = function() {
	...
	if(process.env.NODE_ENV === 'development') {
		...
		console.log("are my changes getting included?");
